...
async fn main() {
    ...
    let mut socket = QuadSocket::connect("localhost:8090").expect("Failed to connect to server");
    #[cfg(target_arch = "wasm32")]
    let mut socket = QuadSocket::connect("ws://localhost:8091").expect("Failed to connect to server");
